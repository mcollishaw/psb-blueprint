...
1. Confirm the opening date and that 32 Byte is locking in the schedule
1. Briefly summarise the solution agreed (in plain, non-technical language appropriate for a practice owner)
1. Mention that formal quotes will follow shortly
1. Invite any questions
1. Close warmly
...
{`PRACTICE SUCCESS BLUEPRINT — INTERNAL BRIEF ${'═'.repeat(50)} Practice: ${d.practiceName||'—'} | ${[d.suburb, d.state].filter(Boolean).join(', ')||'—'} ${d.practiceType==='new'?`Opening: ${d.openingDate||‘TBD’} | Fitout: ${d.fitoutDate||‘TBD’}`:`Go-Live: ${d.goLiveDate||‘TBD’}`} PMS: ${d.pms||'—'} | Imaging SW: ${(d.imagingSw||[]).join(', ')||'—'} Sales Rep: ${d.salesRep||'—'} | Meeting: ${d.meetingDate||'—'} Contact: ${d.contactName||'—'} | ${d.contactEmail||'—'} ${d.financeProvider?`Finance: ${d.financeProvider}`:’’}

${d.practiceType===‘existing’&&d.existingIT?`EXISTING IT PROVIDER ${'─'.repeat(40)} Company: ${d.existingITCompany||'—'} | Type: ${d.existingITType||'—'} Contact: ${d.existingITContact||'—'} | ${d.existingITPhone||'—'} | ${d.existingITEmail||'—'} Contract Expiry: ${d.existingITExpiry||'—'} Currently manages: ${[d.existingITManagesDevices&&'Devices',d.existingITManagesEmail&&'Email',d.existingITManagesPhones&&'Phones',d.existingITManagesInternet&&'Internet',d.existingITManagesSecurity&&'Security'].filter(Boolean).join(', ')||'—'} ${d.existingITNotes?`Notes: ${d.existingITNotes}`:''} `:’’}
SOLUTIONS IN SCOPE
${‘─’.repeat(40)}
...
${d.notes?`MEETING NOTES\n${'─'.repeat(40)}\n${d.notes}`:’’}`} </div> {internalSent && <InfoBox type="info">✅ Internal summary sent to {d.internalTeamEmail}</InfoBox>} <div style={{ display:'flex', gap:10 }}> <button onClick={()=>{ const el=document.createElement('textarea'); el.value=document.querySelector('.psb-internal-summary')?.textContent||''; navigator.clipboard.writeText(d.internalTeamEmail); }} style={{ flex:1, padding:'10px', borderRadius:8, border:`1.5px solid ${C.border}`, background:'transparent', color:C.textSecondary, fontSize:13, fontWeight:600, cursor:'pointer' }}> Copy Summary </button> <button disabled={!d.internalTeamEmail||internalSending||internalSent} onClick={async()=>{ if(!d.internalTeamEmail){alert('Enter a team email address above.');return;} setInternalSending(true); try{ const ejs=await loadEmailJS(); const autoEP=rooms.reduce((a,r)=>a+n(r.qty),0); const ep=n(d.endpoints)||autoEP; await ejs.send(EMAILJS_SERVICE_ID,'YOUR_INTERNAL_TEMPLATE_ID',{ to_email:d.internalTeamEmail, practice:d.practiceName||'New Practice', sales_rep:d.salesRep||'—', go_live:d.practiceType==='new'?(d.openingDate||'TBD'):(d.goLiveDate||'TBD'), practice_type:d.practiceType==='new'?'New build':'Existing / fit-out', summary: `Practice: ${d.practiceName||’—’}, ${d.suburb||’’} ${d.state||’’}
...
return (
<>
<style>{`@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,600&display=swap'); *,*::before,*::after{box-sizing:border-box;margin:0;padding:0} html,body,#root{height:100%} body{font-family:'DM Sans',sans-serif;background:#111827} ::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:rgba(255,255,255,.15);border-radius:3px} input[type=number]::-webkit-inner-spin-button{opacity:.4} button:focus{outline:none} input::placeholder,textarea::placeholder{color:rgba(255,255,255,.25)} select option{background:#1F2937;color:#F9FAFB}`}</style>
